...
| 小卫星非球体 | 已完成 | 火卫一、火卫二走 `irregularRock`，使用不规则几何和低反光材质。 |
...
| `irregularRock` | 不规则小天体型 | `MeshStandardMaterial` | roughness 0.96, no texture | 火卫一、火卫二 |
...
| 火卫一 | 无 | `irregularRock` / `MeshStandardMaterial` | roughness 0.96, low emissive | 无，程序 fallback | normal 无；roughness 无；emissive 无；atmosphere 无；particle 无 | v0.3 已非球体，但材质需更暗更碎石 | 保持不规则几何，低反光，使用 procedural 斑驳 fallback |
| 火卫二 | 无 | `irregularRock` / `MeshStandardMaterial` | roughness 0.96, low emissive | 无，程序 fallback | normal 无；roughness 无；emissive 无；atmosphere 无；particle 无 | 需要与火卫一区分轮廓和色调 | 使用不同 seed 的不规则几何和偏暖暗色 |
...
- Low 模式不加载卫星图片贴图，使用 procedural fallback。
- 火卫一、火卫二不使用图片贴图，避免小天体挂过大资源。
